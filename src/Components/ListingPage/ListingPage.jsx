...
function ListingPage() {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const response = await fetch(`${apiBase}/properties/${id}`);
        if (!response.ok) {
  ...
  if (!property)
    return <div className="min-vh-100 bg-light">Propriété non trouvée</div>;
          ...
          <div className="container pb-4 pt-0">
            <div className="row">
              <div className="col-lg-8">
                ...
                <PropertyMap
                  location={{ lat: property.lat, lng: property.long }}
